...
function getSandboxCredentials(): SandboxCredentials | Record<string, never> {
  ...
  throw new Error(
    `Vercel Sandbox authentication is incomplete. Missing: ${missing.join(
      ", ",
    )}. Set VERCEL_TOKEN, VERCEL_TEAM_ID, and VERCEL_PROJECT_ID in .env.local.`,
  );
}
...
async function createSnapshot() {
  ...
  await run(sandbox, "Validating desktop snapshot dependencies...", "bash", [
    "-c",
    [
      "test -x /usr/local/bin/start-desktop.sh",
      "command -v Xvnc",
